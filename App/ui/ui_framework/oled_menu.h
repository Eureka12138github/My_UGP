...
#define TILES_STARTPOINT_Y			(21)			//磁贴类菜单项开始点相对于屏幕顶部的Y坐标

/***********关于磁贴类菜单项的滚动条开始点相对磁贴底部位置的宏***********/
#define TILES_SCROLLBAR_Y			(4)			//磁贴类菜单项开始点相对于屏幕顶部的Y坐标

/***********关于磁贴类菜单项文字距离屏幕底部的距离的宏***********/
#define TILES_BOTTOM_DISTANCE						(48)			//磁贴类菜单项文字距离屏幕底部的距离
...
#define WINDOW_DATA_STYLE_U16		(6)			//无符号16位整型数据
...
typedef struct MenuWindow{
	...
	uint8_t General_WindowFor;                             //[通用属性]窗口用途
	...
	float* Prob_Data_Float;										//[进度条属性]窗口数据指针(浮点类型)【二选一】								//[进度条属性]窗口数据指针(int16_t整型类型)【五选一】
	u16* Prob_Data_u16;
...
typedef struct MenuItem {
    char* General_item_text;//文本
    void (*General_callback)(void);//返回
    MenuPage* General_SubMenuPage;//下一级

    bool* List_BoolRadioBox;//开关？
