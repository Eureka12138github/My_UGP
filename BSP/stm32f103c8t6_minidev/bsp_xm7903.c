...
// │ 1. 发送查询帧                                     │
// │ 2. 启动一次 DMA 接收                              │
// │ 3. 检查 xm7903_rx_ready：                         │
// │    ├─ 若为 true → 清标志，解析缓冲区，更新有效值  │
// │    └─ 若为 false → 标记数据无效（超时/未响应）    │
// └───────────────────────────────────────────────————┘
// 注意：
// - 本设计假设传感器响应时间 << 任务周期（如 500ms 内返回），
//   因此无需“忙”状态机，每次任务直接重发新查询。
// - 若 DMA 在本次查询前已收到上一轮的响应，仍可被正确处理。
// - 解析逻辑（CRC、字节序等）由外部 XM7903_Parse() 实现。
