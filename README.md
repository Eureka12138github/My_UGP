...
本项目通过 MQTT 协议将传感器数据上报至 OneNet 云平台，使用以下 8 个自定义属性（Property）：

| 名称       | 标识符         | 类型   | 取值范围     | 单位    | 步长      | 用途                             |
|------------|----------------|--------|--------------|---------|-----------|----------------------------------|
| 扬尘       | `dust`         | int32  | 0–1000       | μg/m³   | 1         | PMS7003 实时采集                 |
| 扬尘过大   | `dust_excess`  | bool   | true/false   | —       | —         | 本地判断：`dust > dust_limit`    |
| 扬尘阈值   | `dust_limit`   | int32  | 1–1000       | μg/m³   | 1         | 可通过云平台远程修改             |
| 噪音       | `noise`        | float  | 0.0–100.0    | dB      | 0.1       | XM7903 实时采集                  |
| 噪音过大   | `noise_excess` | bool   | true/false   | —       | —         | 本地判断：`noise > noise_limit`  |
| 噪音阈值   | `noise_limit`  | int32  | 1–120        | dB      | 1         | 可通过云平台远程修改             |
| 湿度       | `humi`         | int32  | 0–100        | %RH     | 1         | DHT11 采集                       |
| 温度       | `temp`         | int32  | 0–100        | °C      | 1         | DHT11 采集                       |
