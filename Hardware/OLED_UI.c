...
void ReverseCoordinate(int16_t X, int16_t Y, int16_t Width, int16_t Height,uint8_t Style){
	switch(Style){
		...
		case REVERSE_ROUNDRECTANGLE://圆角矩形反色
			if(Width >= 4 && Height >= 4){
				OLED_ReverseArea(X, Y, 2, 1);//先把四个边角区域点亮，这里是左上角
				OLED_ReverseArea(X, Y+1, 1, 1);
				OLED_ReverseArea(X+Width-2, Y, 2, 1);//右上角
				OLED_ReverseArea(X+Width-1, Y+1, 1, 1);
				OLED_ReverseArea(X, Y+Height-1, 2, 1);//左下角
				OLED_ReverseArea(X, Y+Height-2, 1, 1);
				OLED_ReverseArea(X+Width-2, Y+Height-1, 2, 1);//右下角
				OLED_ReverseArea(X+Width-1, Y+Height-2, 1, 1);//右下角
			}
			OLED_ReverseArea(X, Y, Width, Height);//再把区域全部反转，此时原来点亮的边角就会熄灭，达到“圆角”效果
			break;
		...
		case UNDERLINE: 
			OLED_ReverseArea(X, Y+Height, Width,1);
...
int16_t CalcStringWidth(int16_t ChineseFont, int16_t ASCIIFont, const char *format, ...) {
	...
	else if(OLED_CHN_CHAR_WIDTH == 3){//新增判断
			while (*ptr != '\0') {
			uint8_t c = (uint8_t)*ptr;
				if ((c & 0xE0) == 0xE0) { 
		//这是根据UTF-8的编码规则作出的判断，3字节：首字节以 1110xxxx 开头（大部分中文字符在此范围
					StringLength += ChineseFont;
					ptr += 3;
				} else if ((c & 0xC0) == 0xC0) { // 2字节字符（如拉丁扩展）
					ptr += 2;
				} else { // ASCII
					StringLength += ASCIIFont;
					ptr++;
...
int8_t GetWindowProbDataStyle(MenuWindow* window){
	...
	if(window->Prob_Data_Float != NULL){
		return WINDOW_DATA_STYLE_FLOAT;
		// 如果窗口数据类型为unsigned int
	}else if(window->Prob_Data_u16 != NULL){
		return WINDOW_DATA_STYLE_U16;
	}else{
		return WINDOW_DATA_STYLE_NONE;
	}
}
		...
		if(DataStyle != WINDOW_DATA_STYLE_NONE ){
			...
			if (DataStyle == WINDOW_DATA_STYLE_FLOAT)
			{
				...
				switch (DataStyle) {
				    ...
				    case WINDOW_DATA_STYLE_U16:

							//计算字符串被限制的最大宽度
							MaxLength = OLED_UI_Window.CurrentArea.Width - 2*CurrentWindow->Text_FontSideDistance - CalcStringWidth(ChineseFont,ASCIIFont,"%d",*CurrentWindow->Prob_Data_u16)- WINDOW_DATA_TEXT_DISTANCE;
							//计算数据所占的宽度
							DataLength = CalcStringWidth(ChineseFont,ASCIIFont,"%d",*CurrentWindow->Prob_Data_u16);
							//显示数据
							OLED_PrintfMixArea(
								//区域限制
								OLED_UI_Window.CurrentArea.X,
...
void SetTargetCursor(void){
	...
	if(CurrentMenuPage->General_MenuType == MENU_TYPE_LIST){
		...
		*(CurrentMenuPage->General_MenuItems[CurrentMenuPage->_ActiveMenuID].General_Value1),
		...
		CurrentMenuPage->General_MenuItems[CurrentMenuPage->_ActiveMenuID].TimeValue->Hour,
		CurrentMenuPage->General_MenuItems[CurrentMenuPage->_ActiveMenuID].TimeValue->Minute,
		CurrentMenuPage->General_MenuItems[CurrentMenuPage->_ActiveMenuID].TimeValue->Second) + 2 + LinePerfixWidth ,
		//当前页面的宽度加当前页面的起始坐标减去开始打印页面起始点的坐标减去6（是滚动条宽度加一）加上行前缀的宽度
		OLED_UI_MenuFrame.CurrentArea.Width + OLED_UI_MenuFrame.CurrentArea.X - OLED_UI_PageStartPoint.CurrentPoint.X - 6 \
		- LinePerfixWidth + LinePerfixWidth - RadioCompensationWidth) ;
	}
	//如果当前页面的类型为Tiles类
	if(CurrentMenuPage->General_MenuType == MENU_TYPE_TILES){
		//磁贴类不需要光标的显示，所以设置为0.
		...
		if(DataStyle == WINDOW_DATA_STYLE_FLOAT){
			...
			switch (DataStyle) {
				case WINDOW_DATA_STYLE_U16:
					...
					OLED_UI_ProbWidth.TargetDistance = ((*CurrentWindow->Prob_Data_u16 - window->Prob_MinData)/((float)window->Prob_MaxData - window->Prob_MinData ))*	(OLED_UI_Window.CurrentArea.Width- 2*window->Prob_SideDistance - 4);
					break;
...
//			page->General_MenuItems[i]._LineSlip = 0; // 焦点切换时显式重置
			...
			*(page->General_MenuItems[i].General_Value1),
			...
			page->General_MenuItems[i].TimeValue->Hour,
			...
			s16 x = TempTargetArea.CurrentArea.X + LinePerfixWidth + CurrentMenuPage->List_StartPointX;
			s16 y = TempTargetArea.CurrentArea.Y;
			s16 width = TempTargetArea.CurrentArea.Width - 6 - LinePerfixWidth - CurrentMenuPage->List_StartPointX - 2 - RadioCompensationWidth;
			s16 height = TempTargetArea.CurrentArea.Height;
			...
			page->General_MenuItems[i].TimeValue->Second);	
				...
				*(page->General_MenuItems[i].General_Value1),//错误类型	
				...
				page->General_MenuItems[i].TimeValue->Minute,
				page->General_MenuItems[i].TimeValue->Second);					
//				if(*page->General_MenuItems[i].ShowErrorMeg == true){
//			
//				}
//				else{//这里主要是为了显示没错误的情况
//				OLED_PrintfMixArea(x,y,width,height,cursorX,cursorY,
//				ChineseFont,ASCIIFont,page->General_MenuItems[i].General_item_text,
//				0,1971,1,1,0,0,0);	//显示无错误加默认Unix时间		
//				}
			}
			//显示 文字+u16 类型的数据	
		...
		//绘制滚动条与其中心线
		OLED_DrawRectangle(TempTargetArea.CurrentArea.X + TempTargetArea.CurrentArea.Width - 5,TempTargetArea.CurrentArea.Y,5,
		(OLED_UI_ScrollBarHeight.CurrentDistance > page->List_MenuArea.Height) ? page->List_MenuArea.Height:OLED_UI_ScrollBarHeight.CurrentDistance,OLED_FILLED);
		
		OLED_DrawLine(TempTargetArea.CurrentArea.X + TempTargetArea.CurrentArea.Width - 3,TempTargetArea.CurrentArea.Y,
		TempTargetArea.CurrentArea.X + TempTargetArea.CurrentArea.Width-3,TempTargetArea.CurrentArea.Height + TempTargetArea.CurrentArea.Y-1);
	}
	//如果当前的页面为磁贴类型
	if(page->General_MenuType == MENU_TYPE_TILES){
		...
		for(MenuID i = 0; i<num;i++){
			...
			//显示磁贴图标
			OLED_ShowImageArea(ceil(CursorPoint.X),CursorPoint.Y,page->Tiles_TileWidth,page->Tiles_TileHeight,0,0,page->Tiles_ScreenWidth,
			page->Tiles_ScreenHeight,page->General_MenuItems[i].Tiles_Icon == NULL?UnKnown:page->General_MenuItems[i].Tiles_Icon);
			// 打印光标右移
			CursorPoint.X += (page->Tiles_TileWidth + OLED_UI_LineStep.CurrentDistance);
			//显示菜单项文字
		...
		if(IF_DRAW_ARROR == true){
			//绘制指示箭头
			OLED_ShowImageArea(page->Tiles_ScreenWidth/2 - 3,TILES_STARTPOINT_Y-6,6,5,0,0,page->Tiles_ScreenWidth,page->Tiles_ScreenHeight,Arrow_Down);
//			OLED_ShowImageArea(0,page->Tiles_ScreenHeight/2-3,5,6,0,0,page->Tiles_ScreenWidth,page->Tiles_ScreenHeight,Arrow_Left);
		}
		//记录此轮循环的字符串宽度
		...
		0,//X坐标
		0,//Y坐标
		page->Tiles_ScreenWidth,//宽度
		page->Tiles_ScreenHeight,//高度
		//下面两行是同一个参数
		StringLength > page->Tiles_ScreenWidth ? 0 + page->General_MenuItems[page->_ActiveMenuID]._LineSlip : page->Tiles_ScreenWidth/2 - CalcStringWidth(ChineseFont,
		ASCIIFont,page->General_MenuItems[page->_ActiveMenuID].General_item_text)/2 + page->General_MenuItems[page->_ActiveMenuID]._LineSlip,//字符左上角X坐标
		page->Tiles_ScreenHeight - page->General_FontSize - TILES_BOTTOM_DISTANCE,//字符左上角Y坐标
		ChineseFont,//中文字体大小
		ASCIIFont,//英文字体大小
		page->General_MenuItems[page->_ActiveMenuID].General_item_text//显示的内容
		);
		//绘制滚动条与其中心线
		...
		OLED_DrawLine(0,TILES_STARTPOINT_Y + page->Tiles_TileHeight + TILES_SCROLLBAR_Y + ScrollBarHeight/2,page->Tiles_ScreenWidth-1,
		TILES_STARTPOINT_Y + page->Tiles_TileHeight + TILES_SCROLLBAR_Y + ScrollBarHeight/2);
	...
	if(DataStyle != WINDOW_DATA_STYLE_NONE){
		...
		if(DataStyle == WINDOW_DATA_STYLE_FLOAT){
			*CurrentWindow->Prob_Data_Float += (WindowProbDeltaData * CurrentWindow->Prob_DataStep);
			...
			switch (DataStyle) {
				case WINDOW_DATA_STYLE_U16:      
					*CurrentWindow->Prob_Data_u16 += (WindowProbDeltaData * CurrentWindow->Prob_DataStep);      
//					if(CurrentWindow->General_WindowFor){Store_Data[CurrentWindow->General_WindowFor] = *CurrentWindow->Prob_Data_u16;Store_Save();}//将新值存储到FLASH中
					if(*CurrentWindow->Prob_Data_u16 <= CurrentWindow->Prob_MinData) {*CurrentWindow->Prob_Data_u16 = CurrentWindow->Prob_MinData;}
					if(*CurrentWindow->Prob_Data_u16 >= CurrentWindow->Prob_MaxData) {*CurrentWindow->Prob_Data_u16 = CurrentWindow->Prob_MaxData;}
					break;					
